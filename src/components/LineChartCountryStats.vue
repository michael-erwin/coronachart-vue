  ...
  >
    <v-card>
      <v-toolbar dense elevation="1" color="primary" dark>
        <v-toolbar-title>Stats - {{country}}</v-toolbar-title>
        <v-spacer />
    ...
    fetchCountryStats () {
      ...
      .then((response) => {
        ...
        for (const item of data.items) {
          const { date, total_confirmed, total_deaths, total_recovered, total_active } = item
          const [ yyyy, mm, dd ] = date.split('-')
          const datestring = `${mm}/${dd}/${yyyy}`
          confirmed_set.data.push({ x: datestring, y: total_confirmed })
          deaths_set.data.push({ x: datestring, y: total_deaths })
          recovered_set.data.push({ x: datestring, y: total_recovered })
          active_set.data.push({ x: datestring, y: total_active })
        }
        // Add datasets
