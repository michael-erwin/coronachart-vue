export default {
  ...
  methods: {
    ...
    fetchCountriesStats (type='count') {
      ...
      this.$axios.get(`/v1.0/covid-stats/countries`, { params })
      .then((r) => {
        if (r) {
    ...
    fetchWorldStats (type='advanced') {
      ...
      let output = false
      const params = { type, origin: this.origin, level: this.level }
      this.$axios.get(`/v1.0/covid-stats/world`, { params })
      .then((r) => {
        if (r) {
